...
50) Find the Number of Ways to Place People I

[leetcode](https://leetcode.com/problems/find-the-number-of-ways-to-place-people-i/description/)
    ...
    def numberOfPairs(self, points):
        ...
        for i in range(n):
            ...
            for j in range(n):
                ...
                # A is upper-left of B (with border)
                if x1 <= x2 and y1 >= y2 and (x1 != x2 or y1 != y2):
                    # Check for other points inside/on rectangle
                    valid = True
                    for k in range(n):
                        if k == i or k == j:
                            continue
...
ll.insert_at_beginning(5)
ll.insert_after(ll.head.next, 15)  # insert after second node
